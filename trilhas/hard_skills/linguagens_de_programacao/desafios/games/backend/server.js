...
const { makeCalculadora } = require('./calculadora')
const { addGame, getGames } = require('./driveDatabaseGames')
const PORT = 3002;
...
app.get('/result_operation/:operation/:operand1/:operand2', (req, res) => {
  const calculadora = makeCalculadora();
  calculadora.setOperand1(Number(req.params.operand1));
  calculadora.setOperand2(Number(req.params.operand2));
  calculadora.setOperation(req.params.operation);
  res.json({
